...
function EmptyState({ darkMode }) {
  return (
    ...
    >
      ...
      >
        This chart will automatically populate once
        enough activity has been recorded.
      </p>
    </div>
  );
}
...
export function HeatmapCard({ title, data = [], darkMode }) {
          ...
          <div key={day} className="grid grid-cols-[42px_1fr] items-center gap-2">
            ...
            >
              {Array.from({ length: 24 }, (_, hour) => {
                const row = data.find(
                  (item) => Number(item.weekday) === dayIndex && Number(item.hour) === hour,
                );
